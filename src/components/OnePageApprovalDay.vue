  ...
  <q-dialog v-model="prompt" full-width>
    <q-card class="editform q-pa-md">
      ...
      <div class="row w-100 justify-end">
        <q-input outlined readonly v-model="editform.month" label="月" class="q-ma-md"/>
        <q-input outlined readonly v-model="editform.day" label="日" class="q-ma-md"/>
        <q-input outlined readonly v-model="editform.onClockIn" label="打卡上班" class="q-ma-md"/>
        <q-input outlined readonly v-model="editform.onClockOut" label="打卡下班" class="q-ma-md"/>
        <q-space/>
        <q-input outlined class="col-3 q-ma-md" readonly v-model="editform._id" autofocus  />
        <q-btn size="20px" color="red" label="取消" v-close-popup class="q-ma-xs" />
        <q-btn size="20px" color="primary" label="修改" v-close-popup @click="Submittime" class="q-ma-xs" />
      </div>
    </q-card>
  </q-dialog>
...
const select = ref({
  ...
  day: today.value.day.toString().padStart(2, '0')
})
const rows = reactive([])
...
async function exportTable () {
  ...
  // 輸出CSV文件的代碼
  const status = exportFile(`${rows[0].name}-${route.params.number}-${rows[0].month}月.csv`, '\ufeff' + fullContent, 'text/csv')
...
const columns = [
  ...
  { name: 'week', label: '星期', field: 'week', align: 'center', sortable: true, style: 'border-right: 2px solid #ddd' },
  {
    name: 'onClockIn',
    label: '打卡上班',
    field: 'onClockIn',
