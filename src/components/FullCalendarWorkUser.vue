...
// --------------------------------------------------------------------
onBeforeMount(async () => {
  const result = await apiAuth.get(`/workschedule/${user.number}`)
  const events = result.data.result.map((item) => ({
    title: `時間 : ${item.ClockIn} - ${item.ClockOut} ${item.place} `,
...
const searchmonth = async () => {
  const result = await apiAuth.post(`/workschedule/month/${user.number}`, selected.value)
...
const searchyear = async () => {
  const result = await apiAuth.post(`/workschedule/year/${user.number}`, selected.value)
  const events = result.data.message.map((item) => ({
    title: `時間 : ${item.onClockIn} - ${item.onClockOut}`,
    date: `${item.year}-${item.month.padStart(2, '0')}-${item.day.padStart(2, '0')}`
  }))
  calendarOptions.value.events = events
}
